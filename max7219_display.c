...
void max7219_Init(void)
{
  ...
  max7219_sendData(0x0A, 5); // intensity
...
{
  uint8_t rotateSymbol[8];
  for (uint8_t i = 0; i < 8; i++)
  {
    rotateSymbol[i]=symbol[c][i];
  }
  rotateChar(rotateSymbol);
  for (uint8_t j = 0; j < 8; j++)
  {
    shiftBuffer(rotateSymbol[j]);
    displayBuffer();
    _delay_ms(140);
  }
}
...
void setPixel(uint8_t x, uint8_t y, char value)
{
  uint8_t xx = y - 1;
  uint8_t yy = (8 - x);
  buffer[xx] = (buffer[xx] & ~(1 << yy)) | (value << yy);
}
